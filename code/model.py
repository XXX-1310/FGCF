...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
    ...
    def computer(self):
        ...
        for layer in range(self.n_layers):
            ...
            s_emb = self.degree_ho * side_emb_ho + self.degree_he * side_emb_he
    ...
    def bpr_loss(self, users, pos, neg):
        ...
        pos_scores = torch.mul(users_emb, pos_emb) + 0.1 * torch.mul(users_emb_he, pos_emb_he) + \
                     0.1 * torch.mul(users_emb_ho, pos_emb_ho)
        pos_scores = torch.sum(pos_scores, dim=1)
        neg_scores = torch.mul(users_emb, neg_emb) + 0.1 * torch.mul(users_emb_he, neg_emb_he) + \
                     0.1 * torch.mul(users_emb_ho, neg_emb_ho)
